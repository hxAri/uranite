...
- `Throwable extends Droper` - the throwable interface now inherits the `Droper` contract, giving every error, exception, and warning an automatic scope-exit `drop()` obligation; `Error`, `Exception`, and `Warning` implement `drop()` releasing their captured traceback through the CRT helper, with subclasses (`AllocationError`, `IOError`, `FileNotFoundError`, etc.) inheriting the implementation
...
- Caught throwables captured into typed locals or received as parameters are released automatically when the owning reference leaves scope, is moved, or is explicitly deleted - previously every traceback captured at raise time (frames array, per-frame records, container) stayed resident for the process lifetime unless manually released; move semantics clear the source alive-flag so ownership transfer does not double-release
